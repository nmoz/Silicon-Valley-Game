...
function preload() {
	...
	musicOnIconImg = loadImage("images/icons/MUSICON_ICON.png");
	musicOffIconImg = loadImage("images/icons/MUSICOFF_ICON.png");
	...
	plane1Img = loadImage("images/characters/plane.png")
...
function setup() {
	...
	MUSICON_ICON = createSprite(windowWidth - 360, windowHeight - 50, 10, 10);
	MUSICON_ICON.addImage("on", musicOnIconImg);
	MUSICON_ICON.scale = 0.155;

	MUSICOFF_ICON = createSprite(windowWidth - 280, windowHeight - 50, 10, 10);
	MUSICOFF_ICON.addImage("off", musicOffIconImg);
	MUSICOFF_ICON.scale = 0.155;
	...
	setTimeout(function() {Plane()}, 20000);

	console.log(platforms);

	//CREATING THE GROUPS//
	planeGroup = new Group();
	planeGroup1 = new Group();
	bombGroup = new Group();
...
function setup1() {
	...
	rocks[0] = new Rock(windowWidth/2, windowHeight);
	platforms[0] = new Platform(random(500, windowWidth - 50), (150, windowHeight - 200));
	enemies[0] = new Enemy(platforms[0].body.position.x + 30, platforms[0].body.position.y - 50);
	cannonBalls[0] = new CannonBall(enemies[0].body.position.x - 68, enemies[0].body.position.y + 7);
...
function draw() {
	...
	if (gameState === HOME) {
		...
		//MOUSE OVER MUSIC BUTTON//
		if (mouseIsOver(MUSICON_ICON)) {
			MUSICON_ICON.scale = 0.165;
		} else {
			MUSICON_ICON.scale = 0.155;
		}

		//MOUSE OVER MUSIC BUTTON//
		if (mouseIsOver(MUSICOFF_ICON)) {
			MUSICOFF_ICON.scale = 0.165;
		} else {
			MUSICOFF_ICON.scale = 0.155;
		}
	...
	if (gameState === PLAY) {
		...
		PLAY_ICON.visible = false;
		MENU_ICON.visible = false;
		MUSICOFF_ICON.visible = false;
		MUSICON_ICON.visible = false;
